...
резолвят исход СЛЕДУЮЩЕЙ свечой после входа, а не фактическим
достижением `stopLoss`/`takeProfit` — это сознательное, синхронизированное
с демо-счётом поведение всего движка (не специфично для гармоник).
Следствие: любой бэктест конкретного детектора паттернов через
`simulate()` измеряет **точность краткосрочного предсказания
направления**, а не качество структурных уровней SL/TP, которые считает
этот детектор (`harmonicStop`/`harmonicTarget` у harmonic-pattern,
аналогичные уровни у Order Block/FVG-детекторов). Не делайте из текущих
R-метрик вывод о "прибыльности SL/TP-логики" — для такой оценки нужен
отдельный режим резолвинга исхода (первое касание TP/SL вместо
направления следующей свечи), которого пока нет.
